...
esp_err_t PCA9685::init_pca(i2c_port_t i2c_port, gpio_num_t sda, gpio_num_t scl, uint32_t freq_hz) {
    _i2c_port = i2c_port;
...
void PCA9685::set_PWM_freq(float freq) {
    ...
    uint8_t oldmode = read_register(MODE1_REG);
    uint8_t newmode = (oldmode & 0x7F) | 0x10;
    write_register(MODE1_REG, newmode);
    write_register(PRESCALE_REG, prescale);
    write_register(MODE1_REG, oldmode);
    vTaskDelay(pdMS_TO_TICKS(5));
    write_register(MODE1_REG, oldmode | 0xA1);
}

esp_err_t PCA9685::write_register(uint8_t reg, uint8_t value) {
